    ...
    def start(self):
        while True:
            try:
                ...
                os._exit(0)
            ctypes.windll.kernel32.SetConsoleTitleW(f'ValChecker {self.version} by nyax44')
            os.system('cls')
            introtext='''
...
░░░╚═╝░░░╚═╝░░╚═╝╚══════╝░╚════╝░╚═╝░░╚═╝╚══════╝░╚════╝░╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝'''
            ...
            print(sys.center('\nhttps://github.com/nyax44/Valorant-Account-Checker\n'))
            res = inquirer.select(
            ...
            if res==menu_choices[1]:
                ...
                os.system('cls')
                print(f'''
    valchecker v{self.version} by nyax44
  ...
  [~] - press ENTER to return
                ...
                with open (str(filename), 'r', encoding='UTF-8',errors='replace') as file:
                    ...
                    for logpass in lines:
                        ...
                        if logpass not in ret and ':' in logpass:
                            self.count+=1
                            ctypes.windll.kernel32.SetConsoleTitleW(f'ValChecker {self.version} by nyax44 | Loading Accounts ({self.count})')
                            ret.append(logpass)
                    return ret
    ...
    def main(self,redirect=False):
        ctypes.windll.kernel32.SetConsoleTitleW(f'ValChecker {self.version} by nyax44 | Loading Settings')
        print('loading settings')
        settings=sys.load_settings()
        ctypes.windll.kernel32.SetConsoleTitleW(f'ValChecker {self.version} by nyax44 | Loading Proxies')
        ...
        if redirect==True:
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(f'ValChecker {self.version} by nyax44 | Loading Checker')
